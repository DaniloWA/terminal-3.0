...
import { regexGerarNumeroTelefone , regexVirgula , numeroPortugues, numeroBraziliero, numeroEspanhol} from "../regex/regex.js";
...
export function gerarNumeroTell(){
    ...
    if( regexGerarNumeroTelefone().exec(dados)){
        if(!info){
            info = true
            adcElemento('info','Experimenta: gerar numero (brasilero|brasil) ou (espanhol|espanha) ou (portugues|portugal)')
        }
        let numeroTelefone1 = []
        let numeroTelefone2 = []
        let numeroTelefone3 = []
        ...
        string = indicativo +  numeroTratado1 + "-" + numeroTratado2 + "-" + numeroTratado3
