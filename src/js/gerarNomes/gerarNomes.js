...
export function gerarNomes() {
  ...
  if (regexGerarNomes().exec(dados)) {
    if (
      regexGerarNomes().exec(dados)[1]?.includes("masculino") ||
      regexGerarNomes().exec(dados)[1]?.includes("feminino")
    ) {
    ...
    if (vezes > 20) {
      ...
      if (
        regexGerarNomes().exec(dados)[1] == undefined &&
        regexGerarNomes().exec(dados)[2] == undefined
      ) {
